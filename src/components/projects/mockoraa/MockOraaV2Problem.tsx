                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-10 sm:mb-12">
                        {stats.map((stat, index) => (
                            <div
                                key={index}
                                className={`bg-gray-50 border border-gray-100 p-6 sm:p-8 rounded-lg transition-all duration-700 hover:border-gray-300 hover:shadow-md ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                                style={{ transitionDelay: `${index * 100}ms` }}
                            >
                                <span className="block text-3xl sm:text-4xl md:text-5xl font-grotesk font-bold text-black mb-2 sm:mb-3">{stat.number}</span>
                                <span className="text-xs sm:text-sm font-inter text-gray-600 leading-relaxed">{stat.desc}</span>
                            </div>
                        ))}
                    </div>

                    <div className="bg-gray-50 border border-red-500 bg-red-500/10 p-6 sm:p-8 rounded-lg border-l-4 border-red-500 shadow-md">
                        <p className="text-lg sm:text-xl md:text-2xl font-grotesk font-bold text-black mb-3 sm:mb-4">
